...
if /I "%c%" EQU "/schedule" goto :/schedule
if /I "%c%" EQU "/lightmode" goto :/lightmode
if /I "%c%" EQU "/weather" goto :/weather
if /I "%c%" EQU "/cmdm" goto :/cmdm
if /I "%c%" EQU "/runcmd" goto :/runcmd
if /I "%c%" EQU "/%user_command_name%" goto :/custom_cmd
...
echo /date: shows today's date.
echo /time: shows the current time (will need to be run again to refresh clock).
...
echo /randnum: generates a random number based on user input.
echo /schedule: lets you loop certain tasks. Add "loop" after running /schedule for continuous looping of commands like /clr, /time, /ip, /date. Example: /dateloop. (must be run after running /schedule)
echo /lightmode: allows you to change the command prompt from light to dark mode.
echo /weather: shows you the weather of your current location.
echo /cmdm: allows you to create a custom command and run it normally.
goto :run_question
...
:/schedule
set /p interval=Enter the interval in seconds:
set /p task=Enter the task/command:

echo Scheduling task: %task% every %interval% seconds...
:loopinf
...
:/weather
set /p location=What is your city?: 
curl wttr.in/%location%
goto :run_question

:/cmdm
set /p user_command_name=Name your command ex. (example): 
set /p user_command_do=Enter what your command will do ex. (echo example): 
goto :run_question
